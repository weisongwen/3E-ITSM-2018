...
class GNSSEvaluation(QMainWindow): # by timer   For paper NLOS exclusion caused by double-decekr bus
    ...
    def GNSSNavPropCall(self,data): # llh from proposed method  # about 20 Hz
        ...
        print 'self.preTime',self.preTime,'self.GNSSNRAWlosDel.GNSS_Raws[-1].GNSS_time',self.GNSSNRAWlosDel.GNSS_Raws[-1].GNSS_time
        ...
        self.preTime = self.GNSSNRAWlosDel.GNSS_Raws[-1].GNSS_time
    ...
    def PointCloudCall(self,data): # 20 Hz As this function has highest frequency, good for update
        ...
        if((self.navfixAvailable) and (len(self.GNSS_.GNSS_Raws)) and (self.navfixTruth_.header.stamp.secs == self.navfix_.header.stamp.secs) and (self.navfixTruth_.header.stamp.secs == self.GNSS_.GNSS_Raws[-1].GNSS_time)):
            ...
            # plot error
            self.axesCurv_3.plot(self.GNSSTime, self.error, '-*', linewidth='1', color='green', label='HDOP')
            ...
            print 'size of self.SatNum', len(self.SatNum), 'size of self.SatNumProp',len(self.SatNumProp)
            with open('GNSS1.csv', "w") as f: # output the integration positioning error
                writer = csv.writer(f)
                for row in rows:
                    writer.writerow(row)
            self.canvas.draw()
