...
class nlosExclusion():
    ...
    def nlosExclusion_(self, dataPB, dataNG, mode, manuSatLis):
        ...
        if (self.nlExcMode == 'statManu_ExclusionOpenLoop'):
            for saIdx in range(len(self.nlosGNSS_.GNSS_Raws)):
                dangerArea = 1; # if danger area, exclusion is needed
                if( (self.nlosGNSS_.GNSS_Raws[-1].GNSS_time >   47745000) and (self.nlosGNSS_.GNSS_Raws[-1].GNSS_time <   47960000)):
                    # print 'safe area '
                    self.mannuSat = [5,24,89,13,2,88]  #5,24     5,24,89,13,2,88
                if( (self.nlosGNSS_.GNSS_Raws[-1].GNSS_time >   47960000) and (self.nlosGNSS_.GNSS_Raws[-1].GNSS_time <   47990000)):
                    self.mannuSat = [5,90,29,15,95,87,2,88]  # 5,90,29   5,90,29,15,95,87,2,88
                    # print 'safe area '
                if( (self.nlosGNSS_.GNSS_Raws[-1].GNSS_time >   47990000) and (self.nlosGNSS_.GNSS_Raws[-1].GNSS_time <   48114000)):
                    self.mannuSat = [24,2,87,5,88,94,90,89]  # 2,24,5   24,2,87,5,88,94,90,89
                    # print 'safe area '
                # corner start: 176810000
                #corner end: 176834000.0
